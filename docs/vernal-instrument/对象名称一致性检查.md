...
`PLATFORM_NA` 的依据是 JVM agent ABI 与 `Instrumentation` API，不是“Rust 生态有替代品”。
若新增 JVMTI/JVM host 支持，必须重新打开此状态。

<!-- detailed-current-start -->
## vernal-instrument 命名审计范围
...
| 目标目录 | 对象数 | 代表对象 | 约束 |
|---|---:|---|---|
| `crate 根目录` | 1 | `InstrumentationSavingAgent` | `mod.rs` 仅声明和重导出 |

## 逐对象命名判定

| 对象 | 预期 Rust 文件 | 当前文件/依赖 | 判定 | 证据 |
|---|---|---|---|---|
| `org.springframework.instrument.InstrumentationSavingAgent` | `instrumentation_saving_agent.rs` | `—` | `PLATFORM_NA` | 该对象的 premain/agentmain 接口依赖 JVM java.lang.instrument.Instrumentation 与 Java Agent 装载协议；Rust 进程不存在对应 JVM 全局 Instrumentation 实例。 |
