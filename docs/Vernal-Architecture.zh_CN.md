...
- `[已确认]` `vernal-axum` 已提供原生 Router 装配与类型化 Context、组件、
  ...
  Body 绑定 Scope 释放；`vernal-poem` 已提供原生 Middleware/Endpoint、类型化
  提取器与 Body 绑定 Scope 释放；`vernal-tonic` 已提供 Context Interceptor、
  类型化 Request 扩展、`Status` 映射与 Tower 组合。
- `[骨架]` 宏及其余六个框架 Adapter 仍只验证 crate 边界与依赖方向。
...
Middleware，Poem 已增加 Middleware/Endpoint、Request Extension 提取器与
Body 生命周期集成，Tonic 已增加原生 Request/Metadata/Status 与 Tower 集成，
其余六个上游框架 Adapter 仍是可编译描述符。详细合同见
