...
基线：Spring Framework **7.0.8**（spring-expression），已覆盖 7.0 新增的
`IndexAccessor`、`TargetedAccessor`、`CompilableIndexAccessor` 语义。
...
|---|---|---|---|
| Expression | 表达式求值 + 赋值 + 类型查询 | `Expression` trait | ✅ 骨架 |
| ExpressionParser | 解析表达式字符串 | `ExpressionParser` trait | ✅ 骨架 |
| EvaluationContext | 求值上下文（根对象、变量、访问器、转换器） | `EvaluationContext` trait | ✅ 骨架 |
| TypedValue | 类型化值封装 | `TypedValue` struct | ⬜ |
| TypeDescriptor | 类型描述符 | `TypeDescriptor` struct | ⬜ |
