...
- 当前优先债务：MISPLACED=2、MISSING=6、STUB=0、PARTIAL=0、UNVERIFIED=1。
- 每批必须绑定对象清单、目录调整、语义测试、审计命令和回退条件。
<!-- current-migration-contract-end -->

## P0：纠正结构

先处理 2 个 `MISPLACED`，按末两层目录移动对象；拆分聚合公开对象并清空 `mod.rs` 类型定义。迁移前后保持公开 API 重导出兼容。

## P1：补齐对象

按[对象审计](../migration-audit/vernal-aspects.md)顺序补齐 6 个 `MISSING`。一次只迁移一个 Java 对象，并同步中文 Javadoc、测试 oracle 和台账。

## P2：补齐语义

对 1 个 `UNVERIFIED` 以及本地事务、缓存、异步、配置对象链做成功、异常、取消、重入和资源清理测试。依赖复用必须记录 crate、提交、源码符号和集成测试。

## 完成条件

严格已处理数等于 9，且不存在 `MISPLACED`、`MISSING`、`STUB`、`PARTIAL`、`UNVERIFIED`；Cargo 测试和 Clippy 的 warning/ignored 必须如实登记。
