...
> 当前权威检查；统计来自[自动对象审计](../migration-audit/vernal-aspects.md)。
...
1. Java 类型 `PascalCase` 转换为同义 `snake_case.rs`，禁止用 `transactional_aspect.rs` 等聚合文件充数。
2. 包路径保留末两层；`beans/factory/aspectj` 的对象落在 `factory/aspectj/`。
3. 主类型名称、方法参数语义与 Java 对齐。
4. `lib.rs`、`mod.rs` 不定义对象。
5. 当前源码存在多个“一文件多公开对象”及 `support/mod.rs` 定义类型问题，均属于未完成证据。

结论：现有切面数量、测试数量或“可运行”不能替代逐对象验收。
