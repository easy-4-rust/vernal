...
- `[Confirmed]` `vernal-axum` provides native Router assembly and typed
  ...
  `vernal-warp` provides native extension filters and a Tower Service body
  scope; `vernal-salvo` provides a native Hoop, typed Depot access, and
...
- `[Skeleton]` Macros and the remaining four framework adapters still validate
...
Actix Web adds App Data/Extensions and native body-aware middleware; Warp adds
extension filters and an official Tower Service lifecycle; Salvo adds a Hoop,
typed Depot access, and frame/trailer-preserving body lifecycle; Poem adds
...
The remaining four upstream framework adapters are compile-checked descriptors.
