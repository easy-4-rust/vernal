...
//!
//! 对应 Java：`AspectJJCacheConfiguration.java`（`spring-aspects` 模块）
//! 包路径：`org.springframework.cache.aspectj`
//! 核心职责：JCache 版 `@Configuration`——注册 `JCacheCacheAspect` Bean，启用 JSR-107 缓存注解（`@CacheResult`/`@CachePut`/`@CacheRemove`/`@CacheRemoveAll`）。
