//! 全面覆盖测试 - Part 15: 通过 VernalApplicationBuilder 获取真实 StartupReport
//! 以及 configuration_properties 的更多集成测试
...
// ════════════════════════════════════════════════════════════════════
// StartupReport - 通过 VernalApplicationBuilder 获取真实报告
// ════════════════════════════════════════════════════════════════════

#[tokio::test]
async fn startup_report_from_context() {
    let rt = tokio::runtime::Handle::current();
    let builder = VernalApplicationBuilder::new(rt);
    let ctx = builder.build().unwrap();

    // Get the startup report from the context
    let report = ctx.startup_report().await;

    // Test all getters on the real report
    ...
    // Count getters
    let _ = report.aop_plan_count();
    let _ = report.aop_interceptor_count();
    let _ = report.local_aop_plan_count();
    let _ = report.local_aop_interceptor_count();

    // Slice getters
    let _ = report.enabled_features();
    let _ = report.adapters();
    let _ = report.external_dependencies();
    let _ = report.condition_evaluations();
    let _ = report.observations();
    let _ = report.warnings();
    let _ = report.unused_definitions();
}

#[tokio::test]
async fn startup_report_setters_on_real() {
    let rt = tokio::runtime::Handle::current();
    let builder = VernalApplicationBuilder::new(rt);
    let ctx = builder.build().unwrap();

    let mut report = ctx.startup_report().await;

    // Test setters on real report
    report.set_context_state("starting".to_string());
    assert_eq!(report.context_state(), "starting");

    report.set_unused_definitions(vec!["def1".into()]);
    assert_eq!(report.unused_definitions().len(), 1);
...
#[tokio::test]
async fn startup_report_debug_on_real() {
    let rt = tokio::runtime::Handle::current();
    let builder = VernalApplicationBuilder::new(rt);
    let ctx = builder.build().unwrap();
...
#[tokio::test]
async fn startup_report_serialize_on_real() {
    let rt = tokio::runtime::Handle::current();
    let builder = VernalApplicationBuilder::new(rt);
    let ctx = builder.build().unwrap();
...
#[test]
fn startup_report_record_warning_dedup() {
    let mut report = create_test_report();

    // Add warnings in different order
    report.record_warning("c");
    report.record_warning("a");
    report.record_warning("b");

    // Check they're sorted and deduplicated
    let count_before = report.warnings().len();
    assert!(count_before >= 3);

    // Add duplicate
    report.record_warning("a");
    let count_after = report.warnings().len();
