//! 全面覆盖测试 - Part 13: 通过实现 Interceptor trait 测试 advisor 方法
//! 以及 startup_report 的更多 getter 方法
...
// ════════════════════════════════════════════════════════════════════
// 实现 Interceptor trait 用于测试 advisor 方法
// ════════════════════════════════════════════════════════════════════
...
impl vernal_aop::Interceptor for TestInterceptor {
    fn intercept<'a>(
        ...
        // Simply pass through to the next interceptor
...
impl vernal_aop::Pointcut for TestPointcut {
    fn matches(&self, _operation: &vernal_aop::Operation) -> bool {
        true // Match all operations
    }
}

// ════════════════════════════════════════════════════════════════════
// ApplicationModuleRegistrar - 测试 advisor_component 方法
// ════════════════════════════════════════════════════════════════════

#[test]
fn registrar_advisor_component() {
    let mut reg = ApplicationModuleRegistrar::new();
    reg.advisor_component::<TestInterceptor, TestPointcut>(TestPointcut, 10);

    let parts = reg.into_parts();
    assert_eq!(parts.advisor_registrations.len(), 1);
}

#[test]
fn registrar_advisor_component_qualified() {
    use vernal_beans::Qualifier;

    let mut reg = ApplicationModuleRegistrar::new();
    let qualifier = Qualifier::new("test-qualifier").unwrap();
    reg.advisor_component_qualified::<TestInterceptor, TestPointcut>(qualifier, TestPointcut, 10);
...
fn registrar_register_advisor_component() {
    // register_advisor_component requires Component + Interceptor
    // TestInterceptor doesn't implement Component, so skip this test
}
