...
use vernal_ioc::{Component, ComponentDefinition, RegistryBuilder};

#[test]
fn explicit_group_selection_is_deterministic_and_excludes_other_groups() {
    let catalog = LinkedComponentCatalog::discover(["test.core"]).expect("core discovery catalog");
    let names = catalog
        .registrations()
        .iter()
...
fn empty_unknown_and_duplicate_groups_fail_closed() {
    assert_eq!(
        LinkedComponentCatalog::discover::<[&str; 0], &str>([]).expect_err("empty group selection"),
        LinkedComponentCatalogError::EmptySelection
    );
    assert!(matches!(
        LinkedComponentCatalog::discover(["test.missing"]).expect_err("unknown group"),
        LinkedComponentCatalogError::MissingGroup { .. }
    ));
    assert!(matches!(
        LinkedComponentCatalog::discover(["test.duplicate"]).expect_err("duplicate group"),
