...
mod tests {
    ...
    fn advised_new() {
        ...
        assert_eq!(advised.operation().component(), "Service");
    }
    ...
    fn advised_target() {
        ...
        let plan = Arc::new(crate::InvocationPlan::new(op, vec![]));
        let target = Arc::new(42i32);
        let advised = Advised::new(target.clone(), plan);
        assert_eq!(**advised.target(), 42);
    }
    ...
    fn advised_plan() {
        ...
        let plan = Arc::new(crate::InvocationPlan::new(op, vec![]));
        let target = Arc::new(42i32);
        let advised = Advised::new(target, plan.clone());
        assert_eq!(advised.plan().operation().component(), "Service");
    }

    #[tokio::test]
    async fn advised_invoke_empty_plan() {
        let op = Operation::new("Service", "method");
        let plan = Arc::new(crate::InvocationPlan::new(op, vec![]));
        let target = Arc::new(42i32);
        let advised = Advised::new(target, plan);
