...
mod additional_tests {
    ...
    #[test]
    fn constructor_invocation_error_construction_failed() {
        let err = ConstructorInvocationError::ConstructionFailed("test error".to_string());
        assert_eq!(format!("{}", err), "Construction failed: test error");
    }

    #[test]
    fn constructor_invocation_error_argument_mismatch() {
        let err = ConstructorInvocationError::ArgumentMismatch("test error".to_string());
        assert_eq!(format!("{}", err), "Argument mismatch: test error");
    }

    #[test]
    fn constructor_invocation_error_debug() {
        let err = ConstructorInvocationError::ConstructionFailed("test".to_string());
        let debug = format!("{:?}", err);
        assert!(debug.contains("ConstructionFailed"));
    }

    #[test]
    fn constructor_invocation_error_trait() {
        let err = ConstructorInvocationError::ConstructionFailed("test".to_string());
