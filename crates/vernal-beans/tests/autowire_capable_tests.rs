//! AutowireCapableBeanFactory trait 实现的全面覆盖测试。

use std::any::Any;
use std::sync::Arc;

use vernal_beans::{
    AutowireCapableBeanFactory, BeanFactory, ComponentDefinition, Container,
    Qualifier, RegistryBuilder, Resolver, Scope,
};
...
fn build_transient_container() -> Container {
    let mut builder = RegistryBuilder::new();
    builder
        .register(ComponentDefinition::transient::<String, _>(|_resolver: &Resolver| {
            "transient".to_string()
        }))
        .unwrap();
    let registry = builder.build().unwrap();
...
#[test]
fn create_bean_string() {
    let container = build_string_container();
    let result = container.create_bean(std::any::type_name::<String>());
    assert!(result.is_ok());
...
fn create_bean_i32() {
    ...
    let result = container.create_bean(std::any::type_name::<i32>());
    assert!(result.is_ok());
    let value = result.unwrap();
...
#[test]
fn create_bean_transient() {
    let container = build_transient_container();
    let result = container.create_bean(std::any::type_name::<String>());
    assert!(result.is_ok());
    let value = result.unwrap();
    assert_eq!(*value.downcast_ref::<String>().unwrap(), "transient");
}
...
#[test]
fn autowire_bean_i32() {
    let container = build_i32_container();
    let existing: Arc<dyn Any + Send + Sync> = Arc::new(123i32);
    let result = container.autowire_bean(existing);
    assert!(result.is_ok());
}
...
fn autowire_bean_no_matching_definition() {
    let container = build_string_container();
    let existing: Arc<dyn Any + Send + Sync> = Arc::new(Vec::<String>::new());
    let result = container.autowire_bean(existing);
    assert!(result.is_ok());
}
...
#[test]
fn configure_bean_string() {
    let container = build_string_container();
    let existing: Arc<dyn Any + Send + Sync> = Arc::new("test".to_string());
    let result = container.configure_bean(existing, "test_bean");
    assert!(result.is_ok());
}

#[test]
fn configure_bean_i32() {
    let container = build_i32_container();
...
fn configure_bean_with_post_processor() {
    ...
    impl vernal_beans::BeanPostProcessor for TestBPP {
        ...
        {
            // 返回修改后的 bean
            if let Some(s) = bean.downcast_ref::<String>() {
                Ok(Some(Arc::new(format!("processed_{}", s))))
            } else {
                Ok(None)
            }
        }
    ...
    assert!(result.is_ok());
    let value = result.unwrap();
    assert_eq!(*value.downcast_ref::<String>().unwrap(), "processed_test");
}
...
fn autowire_no_mode() {
    let container = build_string_container();
    // AUTOWIRE_NO = 0
    let result = container.autowire(std::any::type_name::<String>(), 0, false);
    assert!(result.is_ok());
    let value = result.unwrap();
    assert_eq!(*value.downcast_ref::<String>().unwrap(), "hello");
}
...
fn autowire_by_name_mode() {
    let container = build_string_container();
    // AUTOWIRE_BY_NAME = 1
    let result = container.autowire(std::any::type_name::<String>(), 1, false);
    assert!(result.is_ok());
}
...
fn autowire_by_type_mode() {
    let container = build_string_container();
    // AUTOWIRE_BY_TYPE = 2
    let result = container.autowire(std::any::type_name::<String>(), 2, false);
    assert!(result.is_ok());
}
...
fn autowire_constructor_mode() {
    ...
    let result = container.autowire(std::any::type_name::<String>(), 3, false);
    assert!(result.is_ok());
}
...
fn autowire_not_found() {
    let container = build_string_container();
    let result = container.autowire("NonExistentClass", 0, false);
    assert!(result.is_err());
}
...
fn initialize_bean() {
    ...
    let result = container.initialize_bean(bean, "test_bean");
    assert!(result.is_ok());
}
...
fn destroy_bean_instance() {
    let container = build_string_container();
    let bean: Arc<dyn Any + Send + Sync> = Arc::new("test".to_string());
    let result = container.destroy_bean_instance("test_bean", &*bean);
    // destroy_bean_instance 返回 Result
    let _ = result;
}
...
fn container_add_multiple_bean_post_processors() {
    ...
    struct TestBPP3;
    impl vernal_beans::BeanPostProcessor for TestBPP3 {}

    container.add_bean_post_processor(Arc::new(TestBPP1));
    container.add_bean_post_processor(Arc::new(TestBPP2));
    container.add_bean_post_processor(Arc::new(TestBPP3));
    assert_eq!(container.bean_post_processor_count(), 3);
}
