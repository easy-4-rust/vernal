...
#[test]
fn container_new() { let c = make_container(); let _ = c.registry(); }

#[test]
fn container_warm_up() { let c = make_container(); assert!(c.warm_up().is_ok()); }

#[test]
fn container_unused_definitions() { let c = make_container(); let _ = c.unused_definitions(); }

#[test]
fn container_open_scope() { let c = make_container(); let _ = c.open_scope::<String>(); }

#[test]
fn container_transient_tracker() { let c = make_container(); let _ = c.transient_tracker(); }
...
fn container_resolve_i32() {
    let c = make_container();
    let val: Result<Arc<i32>, _> = c.resolve();
    assert!(val.is_ok());
    assert_eq!(*val.unwrap(), 42);
}

#[test]
fn container_resolve_f64() {
    let c = make_container();
    let val: Result<Arc<f64>, _> = c.resolve();
    assert!(val.is_ok());
    assert!((*val.unwrap() - 3.14).abs() < f64::EPSILON);
}
...
fn container_resolve_qualified() {
    ...
    assert!(val.is_ok());
    assert_eq!(*val.unwrap(), "primary_val");
}
...
#[test]
fn container_bean_post_processor() {
...
#[test]
fn container_registry() { let c = make_container(); let _ = c.registry(); }
...
fn bean_wrapper_register_property() {
    ...
    wrapper.register_property("age", std::any::TypeId::of::<i32>());
...
// ═══════════════════════════════════════════════════════════════════
// Qualifier, ScopeKey, ComponentKey, TraitKey
// ═══════════════════════════════════════════════════════════════════
