//! 最终覆盖率提升测试：container, scope_context 等核心模块
...
#[test]
fn scope_context_get_or_insert_with_different_types() {
    let container = build_string_container();
    let scope = container.open_scope::<String>();
    let str_result = scope.get_or_insert_with::<String, _>(|| "string".to_string());
    let i32_result = scope.get_or_insert_with::<i32, _>(|| 42);
...
#[test]
fn scope_context_on_close_multiple() {
    let container = build_string_container();
    let scope = container.open_scope::<String>();
    let _ = scope.on_close(move || async { Ok::<(), std::io::Error>(()) });
    let _ = scope.on_close(move || async { Ok::<(), std::io::Error>(()) });
}
...
fn dependency_is_optional() {
    ...
    let opt_dep = vernal_beans::Dependency::optional_of::<String>();
    let _ = opt_dep;
}
