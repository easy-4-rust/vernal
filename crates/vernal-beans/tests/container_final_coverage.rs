...
fn container_add_bean_post_processor() {
    let mut builder = RegistryBuilder::new();
    builder.register(ComponentDefinition::singleton::<String, _>(|_| "hello".to_string())).unwrap();
    let mut container = Container::new(builder.build().unwrap());
    assert_eq!(container.bean_post_processor_count(), 0);
...
fn container_warm_up_multi() {
    ...
    builder.register(ComponentDefinition::singleton::<String, _>(|_| "hello".to_string())).unwrap();
    builder.register(ComponentDefinition::singleton::<i32, _>(|_| 42i32)).unwrap();
    builder.register(ComponentDefinition::singleton::<bool, _>(|_| true)).unwrap();
    let container = Container::new(builder.build().unwrap());
    assert!(container.warm_up().is_ok());
}

// ═══════════════════════════════════════════════════════════════════════════════
// Container BeanFactory trait 方法测试
// ═══════════════════════════════════════════════════════════════════════════════
...
fn component_definition_try_singleton() {
    let def = ComponentDefinition::try_singleton::<String, _>(|_| Ok("test".to_string()));
    assert_eq!(def.scope(), Scope::Singleton);
}

#[test]
fn component_definition_try_transient() {
    let def = ComponentDefinition::try_transient::<String, _>(|_| Ok("test".to_string()));
    assert_eq!(def.scope(), Scope::Transient);
}
...
fn registry_builder_register_duplicate() {
    let mut builder = RegistryBuilder::new();
    builder.register(ComponentDefinition::singleton::<String, _>(|_| "first".to_string())).unwrap();
    let result = builder.register(ComponentDefinition::singleton::<String, _>(|_| "second".to_string()));
    assert!(result.is_err());
}
...
fn resolve_error_scope_not_active() {
    ...
    let err = vernal_beans::ResolveError::ScopeNotActive { component: key, scope: scope_key };
    let msg = format!("{err}");
    assert!(!msg.is_empty());
}
...
fn resolve_error_type_mismatch() {
    let key = vernal_beans::ComponentKey::of::<String>();
    let err = vernal_beans::ResolveError::TypeMismatch { component: key };
    let msg = format!("{err}");
    assert!(!msg.is_empty());
}
...
fn resolve_error_provider_used_during_construction() {
    let key = vernal_beans::ComponentKey::of::<String>();
    let err = vernal_beans::ResolveError::ProviderUsedDuringConstruction { component: key, dependency: "test".to_string() };
    let msg = format!("{err}");
    assert!(!msg.is_empty());
}
...
fn resolve_error_scope_owner_mismatch() {
    let scope_key = vernal_beans::ScopeKey::of::<String>();
    let err = vernal_beans::ResolveError::ScopeOwnerMismatch { scope: scope_key };
    let msg = format!("{err}");
    assert!(!msg.is_empty());
}
...
fn resolve_error_undeclared_dependency() {
    let key = vernal_beans::ComponentKey::of::<String>();
    let err = vernal_beans::ResolveError::UndeclaredDependency { component: key, dependency: "test".to_string() };
    let msg = format!("{err}");
    assert!(!msg.is_empty());
}
...
fn resolve_error_circular_runtime() {
    let err = vernal_beans::ResolveError::CircularRuntime { path: vec!["a".to_string(), "b".to_string(), "a".to_string()] };
    let msg = format!("{err}");
    assert!(!msg.is_empty());
}
...
fn resolve_error_scope_unavailable() {
    let key = vernal_beans::ComponentKey::of::<String>();
    let scope_key = vernal_beans::ScopeKey::of::<String>();
    let err = vernal_beans::ResolveError::ScopeUnavailable { component: key, scope: scope_key, state: vernal_beans::ScopeState::Closed, cancelled: false };
    let msg = format!("{err}");
    assert!(!msg.is_empty());
}
...
fn resolve_error_trait_binding_type_mismatch() {
    ...
    let err = vernal_beans::ResolveError::TraitBindingTypeMismatch { binding: trait_key, target };
    let msg = format!("{err}");
    assert!(!msg.is_empty());
}
...
fn graph_error_missing_dependency() {
    let err = vernal_beans::GraphError::MissingDependency { path: vec!["a".to_string(), "b".to_string()] };
    let msg = format!("{err}");
...
fn graph_error_ambiguous_dependency() {
    let err = vernal_beans::GraphError::AmbiguousDependency { path: vec!["a".to_string()], candidates: vec!["c1".to_string(), "c2".to_string()] };
    let msg = format!("{err}");
...
fn graph_error_missing_trait_binding_target() {
    let err = vernal_beans::GraphError::MissingTraitBindingTarget { binding: "my binding".to_string() };
    let msg = format!("{err}");
...
fn graph_error_cycle() {
    let err = vernal_beans::GraphError::Cycle { path: vec!["a".to_string(), "b".to_string(), "a".to_string()] };
    let msg = format!("{err}");
...
fn dependency_descriptor_for_field() {
    let dd = vernal_beans::DependencyDescriptor::for_field(std::any::TypeId::of::<String>(), std::any::type_name::<String>());
    let _ = dd;
}

#[test]
fn dependency_descriptor_for_constructor_parameter() {
    let dd = vernal_beans::DependencyDescriptor::for_constructor_parameter(0, std::any::TypeId::of::<String>(), std::any::type_name::<String>());
    let _ = dd;
}

#[test]
fn dependency_descriptor_for_method_parameter() {
    let dd = vernal_beans::DependencyDescriptor::for_method_parameter(0, std::any::TypeId::of::<String>(), std::any::type_name::<String>());
    let _ = dd;
}
