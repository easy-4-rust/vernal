...
mod tests {
    ...
    fn set_as_text_stores_value() {
        ...
        assert_eq!(editor.get_as_text(), Some("a,b,c".to_string()));
    }
    ...
    fn set_and_get_value() {
        let mut editor = StringArrayPropertyEditor::new();
        let val: Arc<dyn Any + Send + Sync> = Arc::new("test_array".to_string());
        editor.set_value(val);
        let retrieved = editor.get_value().unwrap();
        assert_eq!(*retrieved.downcast_ref::<String>().unwrap(), "test_array");
    }
