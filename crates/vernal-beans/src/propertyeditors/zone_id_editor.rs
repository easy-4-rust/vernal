//! ZoneIdEditor — Spring 风格的 ZoneId 编辑器。
//!
//! 对应 Java 类：`org.springframework.beans.propertyeditors.ZoneIdEditor`。
...
/// Spring 风格的 ZoneId 编辑器。
pub struct ZoneIdEditor {
    value: Option<String>,
}
...
impl Default for ZoneIdEditor {
    fn default() -> Self {
        Self::new()
    }
}

impl PropertyEditor for ZoneIdEditor {
...
mod tests {
    ...
    #[test]
    fn default_trait() {
        let editor = ZoneIdEditor::default();
        assert!(editor.value.is_none());
    }
    ...
    fn set_as_text_stores_value() {
        ...
        assert_eq!(editor.get_as_text(), Some("Europe/London".to_string()));
    }

    #[test]
    fn set_as_text_trims_whitespace() {
        let mut editor = ZoneIdEditor::new();
        editor.set_as_text("  Asia/Tokyo  ").unwrap();
