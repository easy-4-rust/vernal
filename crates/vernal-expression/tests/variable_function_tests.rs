...
fn ae(e: &str, ctx: &dyn EvaluationContext) {
    assert!(
...
fn aec(e: &str, ctx: &dyn EvaluationContext, s: &str) {
...
fn var_long() {
    assert!(
        SpelExpressionParser::new()
            .parse_expression("#thisIsAVeryLongVariableName")
            .is_ok()
    );
}
#[test]
fn var_this_root() {
    ...
    let e = SpelExpressionParser::new()
        ...
        .unwrap();
    assert_eq!(
        e.get_value_with_context(&StandardEvaluationContext::new(root.clone()))
            .unwrap()
            .value(),
        root.value()
    );
...
fn var_this_null() {
    let e = SpelExpressionParser::new()
        ...
        .unwrap();
    assert_eq!(
        e.get_value_with_context(&StandardEvaluationContext::new(TypedValue::null()))
            .unwrap()
            .value(),
...
fn var_root_list() {
    ...
    let root = TypedValue::new(ExpressionValue::List(items), TypeDescriptor::OBJECT);
    let e = SpelExpressionParser::new()
        .parse_expression("#root")
        .unwrap();
    assert!(matches!(
        e.get_value_with_context(&StandardEvaluationContext::new(root))
            .unwrap()
            .value(),
...
fn var_set_null() {
    let mut ctx = SimpleEvaluationContext::for_read_only(TypedValue::null());
    ctx.set_variable("empty", TypedValue::null());
    let e = SpelExpressionParser::new()
...
fn var_overwrite() {
    ...
    let e = SpelExpressionParser::new().parse_expression("#x").unwrap();
    assert_eq!(
        e.get_value_with_context(&ctx).unwrap().value(),
...
fn fn_mixed() {
    assert!(
        SpelExpressionParser::new()
            .parse_expression("#process(1, 'two', true)")
            .is_ok()
    );
}
#[test]
...
fn bean_underscore() {
    assert!(
        SpelExpressionParser::new()
            .parse_expression("@_internal_bean")
            .is_ok()
    );
}
#[test]
...
fn var_expr_str() {
    let e = SpelExpressionParser::new()
        .parse_expression("#myVar")
        .unwrap();
    assert_eq!(e.expression_string(), "#myVar");
}
#[test]
fn bean_expr_str() {
    let e = SpelExpressionParser::new()
        .parse_expression("@myBean")
        .unwrap();
    assert_eq!(e.expression_string(), "@myBean");
}
#[test]
fn fn_expr_str() {
    let e = SpelExpressionParser::new()
        .parse_expression("#fn(1, 2)")
        .unwrap();
    let es = e.expression_string();
    assert!(es.contains("#fn") && es.contains("1") && es.contains("2"));
}
