...
fn parse_err(expr: &str) {
    let parser = SpelExpressionParser::new();
    assert!(
        parser.parse_expression(expr).is_err(),
...
#[test]
fn literal_real_float_suffix() {
    assert_eq!(eval("3.14F"), ExpressionValue::Float(3.14));
}

#[test]
fn literal_real_double_suffix() {
    assert_eq!(eval("3.14D"), ExpressionValue::Float(3.14));
}
...
#[test]
fn literal_scientific() {
    assert_eq!(eval("1e3"), ExpressionValue::Float(1000.0));
}

#[test]
fn literal_scientific_negative() {
...
#[test]
fn string_int_concat() {
    assert_eq!(
        eval("'value: ' + 42"),
        ExpressionValue::String("value: 42".to_string())
    );
}

#[test]
fn int_string_concat() {
    assert_eq!(
        eval("42 + ' is the answer'"),
        ExpressionValue::String("42 is the answer".to_string())
    );
}
...
#[test]
fn ternary_false() {
    assert_eq!(eval("false ? 1 : 2"), ExpressionValue::Int(2));
}
...
#[test]
fn instanceof_parse() {
    parse_ok("'hello' instanceof T(String)");
}
