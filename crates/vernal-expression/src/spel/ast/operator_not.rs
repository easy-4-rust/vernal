...
pub struct OperatorNot { operand: Box<dyn SpelNode> }

impl OperatorNot {
    #[must_use] pub fn new(operand: Box<dyn SpelNode>) -> Self { Self { operand } }
}
...
impl SpelNode for OperatorNot {
    fn get_value(&self, context: &dyn EvaluationContext) -> Result<TypedValue, EvaluationException> {
        ...
        let result = match value.value() {
            ...
            _ => return Err(EvaluationException::new("", None, "逻辑非运算不支持的操作数类型")),
        };
