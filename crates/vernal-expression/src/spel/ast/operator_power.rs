...
impl SpelNode for OperatorPower {
...
impl BinaryOperator for OperatorPower {
    ...
    fn operate(&self, left: &TypedValue, right: &TypedValue) -> Result<TypedValue, EvaluationException> {
        match (left.value(), right.value()) {
            (ExpressionValue::Int(l), ExpressionValue::Int(r)) => {
                Ok(TypedValue::new(ExpressionValue::Int(l.pow(*r as u32)), TypeDescriptor::INT))
            }
            (ExpressionValue::Float(l), ExpressionValue::Float(r)) => {
                Ok(TypedValue::new(ExpressionValue::Float(l.powf(*r)), TypeDescriptor::FLOAT))
            }
            _ => Err(EvaluationException::new("", None, "幂运算不支持的操作数类型")),
        }
    }
}
