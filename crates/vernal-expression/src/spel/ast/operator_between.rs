...
impl SpelNode for OperatorBetween {
    fn get_value(&self, context: &dyn EvaluationContext) -> Result<TypedValue, EvaluationException> {
        ...
        let result = match (value.value(), low.value(), high.value()) {
            (ExpressionValue::Int(v), ExpressionValue::Int(l), ExpressionValue::Int(h)) => v >= l && v <= h,
            (ExpressionValue::Float(v), ExpressionValue::Float(l), ExpressionValue::Float(h)) => v >= l && v <= h,
            (ExpressionValue::String(v), ExpressionValue::String(l), ExpressionValue::String(h)) => v.as_str() >= l.as_str() && v.as_str() <= h.as_str(),
            _ => return Err(EvaluationException::new("", None, "between 运算要求左右操作数类型一致")),
        };
        Ok(TypedValue::new(ExpressionValue::Boolean(result), TypeDescriptor::BOOLEAN))
    }

    fn to_string_ast(&self) -> String {
        format!("({} between {} and {})", self.value.to_string_ast(), self.low.to_string_ast(), self.high.to_string_ast())
    }
}
