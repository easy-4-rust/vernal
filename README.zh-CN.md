...
> 和合同测试；Axum、Actix Web、Salvo、Poem 与 Tonic 已具备可运行适配，过程宏
> 及其余五个 Adapter 仍是骨架。当前尚未发布。
...
`Transform`/`Service`；Salvo 已提供原生 Hoop、类型化 Depot 访问以及
Frame/Trailer 保真的 Body 释放；Poem 已提供原生 `Middleware`/`Endpoint`
组合、类型化 Context/组件/Scope 提取器和 Body 绑定释放；Tonic 已提供 Context
